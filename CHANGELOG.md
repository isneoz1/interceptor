...
- **A WebSocket session that is still receiving now grows as you watch it.** The detail
  panel was drawn when you opened a row and nothing refreshed it: `onDelta` redrew the
  *views*, never the open panel. To see what had arrived since, you had to close the row and
  open it again — which, for a game or a market feed, meant never really seeing it.

  Frames are **appended**, not redrawn. Redrawing would restart the batch render and send
  the reader back to the first 150 of them; on a fifty-thousand-frame session, "following"
  would have meant returning to the beginning at every new arrival.

  And it only follows while you are **at the bottom of the panel**. Someone reading further
  up is never moved under their eyes. It is the behaviour of a log you are tailing, and the
  one the request table already applies with its **Follow** button. At most three refreshes
  a second.

- **The anti-CSP fallback captures WebSocket frames.** When a page's security policy
  forbids injecting a script — which is the case on many games, banks and hardened sites —
  the bridge falls back to Firefox's Xray vision, which does not depend on it. That fallback
  covered `fetch`, XHR and `sendBeacon`, and said so itself: *"WebSocket frames unavailable
  on this document"*. It was true, and it was exactly what was missing for "every site".

  The constructor cannot be replaced there — an exported function called with `new` would
  not return a real WebSocket — but it does not need to be. To receive anything a page must
  register a handler, and there are only two ways: `onmessage` or
  `addEventListener('message')`. To send, it goes through `send`. All three live on the
  prototype and are reachable under Xray.

  **This path exists only in Firefox**: `wrappedJSObject` and `exportFunction` belong to its
  engine. The audits in this repository run in Chromium and therefore cannot exercise it —
  said here rather than left to be assumed.

- **No WebSocket escapes the probe any more, whatever its age.** The `Proxy` on the
  constructor sees everything built *after* us. A connection opened before — a delayed
  injection, a document already alive, a script that ran faster than we did — slipped
  through entirely: neither its outgoing nor its incoming frames.

  Since a page must register a handler to receive, and both ways go through the prototype, a
  connection that escaped us is now adopted the moment it is used. `tools/sockets.mjs` opens
  one **before** loading the probe and checks its frames are captured all the same.

1202 assertions across six suites, plus four browser audits.

## [4.2.0] — 2026-09-17

### Fixed

- **The probes were detectable, and that is a bug in itself.** The promise at the top of
  `content/hooks.js` is that they are *purely passive*. A page that notices `fetch` has been
  replaced is no longer being observed — it is being **changed**. Plenty of sites check
  exactly this and behave differently, or refuse to run, when they find an altered function.

  Measured by having a page take stock of itself, installing the probes, and taking stock
  again:

  | What the page could see | Before | After |
  |---|---|---|
  | `fetch` name, length, prototype | `fetch\|1\|undefined` | `\|2\|object` |
  | `XMLHttpRequest.open` | `open\|2\|undefined` | `\|3\|object` |
  | `XMLHttpRequest.send` | `send\|0\|undefined` | `\|1\|object` |
  | `navigator.sendBeacon` | `sendBeacon\|1\|undefined` | `\|2\|object` |
  | `WebSocket.prototype.send` | `send\|1\|undefined` | `\|1\|object` |
  | `[native code]` in `toString()` | yes | **no** |

  Three giveaways, each enough on its own: the **name** disappeared, the **length** changed
  because the probe declared its own parameters, and a **`prototype`** appeared — a native
  method has none, an ordinary function does, and `delete f.prototype` cannot remove it
  since the property is not configurable.

  The six replaced functions are now built in a form that has no prototype, carry the name
  and length of what they replace, and `Function.prototype.toString` returns the original
  text. The constructors — `WebSocket`, `Worker`, `RTCPeerConnection`, `EventSource` — never
  had the problem: a `Proxy` already forwards its target's name, length and text.

  The presence flag on `window` is non-enumerable now, so page code that walks `window` does
  not trip over a property that would not be there without us.

### Added

- **`tools/transparence.mjs`** — has a page measure itself before and after the probes are
  installed, and compares 42 observations: names, lengths and prototypes, `instanceof` and
  prototype chains, `[native code]`, property descriptors, the type of error thrown by
  malformed calls, subclassing, and return values for awkward call shapes. Anything that
  differs is a finding. It also checks the probes actually ran — an audit that breaks
  nothing because it installed nothing proves nothing.

4.2.0 is the version number that was asked for; it skips 4.0 and 4.1.

1191 assertions across six suites, plus four browser audits.

## [3.13.0] — 2026-09-17

### Added

- **A binary WebSocket frame now carries its bytes.** It used to carry only its size:
  `binary, 42 B`, and nothing else. For a binary protocol — protobuf, MessagePack, CBOR,
  all common over WebSocket — that was everything you got, while the toolbox has been able
  to read all three from the start.

  The bytes are kept now, bounded by the same setting as text (`maxFrameBytes`), and the
  Streams tab reads them: it names the format when one of the three readers recognises it,
  shows the opening bytes in hexadecimal, and a click drops the frame into the toolbox. A
  frame none of them recognises is shown as bytes and nothing more — no format is invented.

  `Blob` payloads are read too, asynchronously, keeping the moment the frame occurred so
  the order stays true.

- **WebRTC data channels are captured.** The probe noted that a channel had been *created*
  and nothing else. A `RTCDataChannel` is exactly a socket: bidirectional, carrying text and
  bytes. Each channel is now a row of its own with its frames — same display, same binary
  reading, same counters — on both sides: the channel the page opens and the one the peer
  opens. It says what it is rather than posing as a WebSocket.

- **WebTransport carries its data.** Open and close were seen; what went through was not.
  Outgoing datagrams are captured by wrapping the writer, incoming ones through `tee()` —
  which splits the stream so the page receives exactly what it would have received without
  us — and stream creation is counted, with its direction.

- **WebSocket frames inside workers**, behind a setting that is **off by default**.

  Worth stating plainly: the **handshake** of every WebSocket was already captured wherever
  it was opened — page, `Worker`, `SharedWorker`, Service Worker — because `webRequest` sees
  it at the network layer. URL, headers, status 101 and timing were never missing. What was
  missing were the **frames** of those opened outside the page context, since the probe
  replaces `window.WebSocket` and a worker has its own `self.WebSocket`.

  Reaching inside means loading the worker script from a Blob that installs the probe first.
  One thing then changes and the page can see it: `self.location` becomes the Blob URL.
  Relative resolution is corrected — `importScripts`, `fetch` and `XMLHttpRequest` resolve
  against the original URL — but a worker that reads `self.location` itself will see
  something else. Hence the setting, off by default, saying what it costs. If the Blob is
  refused, by a strict `worker-src` policy for instance, the original URL is used and the
  site starts normally, simply without the probe.

- **`tools/sockets.mjs`** starts a real WebSocket server — RFC 6455 handshake and framing,
  fifty lines, no dependency — and proves all of the above against it: a connection from the
  page, one from a `Worker`, one from a `SharedWorker`, and a WebRTC data channel between
  two peer connections. It also checks the page is **not disturbed**: its worker still
  receives its own messages, and a relative `importScripts` inside that worker still
  resolves.

1191 assertions across six suites.

## [3.12.0] — 2026-09-14

### Changed

- **The heaviest surfaces are between 7 and 86 times faster.** Measured in Chrome with the
  detail panel actually open — a closed panel measures zero, which is how these costs went
  unnoticed for so long.

  | | Before | After |
  |---|---|---|
  | A 5 MB response body | **2 159 ms** | 25 ms |
  | A 1 MB response body | 289 ms | 35 ms |
  | Streams tab, 20 000 WebSocket frames | 92 ms, 60 015 nodes | 3 ms, 467 nodes |
  | Timeline tab, 5 000 events | 16 ms | 3 ms |
  | Sites and paths, 20 000 requests | 126 ms | 17 ms |

  Nothing is hidden and nothing is dropped:

  - **A large body shows its first 200 000 characters and says so**, with a button to show
    all of it. The figure is not arbitrary: a `<pre>` costs about a quarter of a
    millisecond per kilobyte, so that is where the panel stops feeling instant. **Copy** and
    **Toolbox** still work on the whole body — only the view is bounded, never the data.
    Formatting is no longer applied on its own past that size either: it re-reads and
    rewrites the entire body, which on 5 MB costs 50 ms by itself.
  - **WebSocket frames, SSE messages and timeline events render in batches.** None of the
    three is capped by any setting — `maxWebSocketFrames` defaults to unlimited — so a long
    session accumulates tens of thousands. The rest arrives as you scroll, and a test
    scrolls to the end to confirm all 20 000 come back.
  - **Sites and paths no longer builds what is collapsed.** A closed `<details>` shows
    nothing; its contents are built the moment it is opened. Opening fills on the click
    itself, before the browser paints, so a branch is never briefly empty.

- **A batch list waits until it is in the page before observing.** An `IntersectionObserver`
  requires its root to already be an ancestor of what it observes; a root that is not one
  yet reports "never visible" **permanently** — the intersection is not recomputed when the
  ancestry appears. The detail panel builds its tabs detached and inserts them afterwards,
  which is exactly that case: the frame list stopped at 150 of 20 000. The observer now
  waits for insertion, so the real scrolling frame is known and the 600 px margin means
  something again.

1185 assertions across six suites.

## [3.11.0] — 2026-09-12

### Fixed

- **Three views could spin forever and freeze the tab.** The shape was the same in each:
  `render()` sees its state empty, asks the kernel, and the answer comes back carrying
  nothing — no error either. The state stays empty, the answer triggers another render, and
  round it goes.

  The loop runs entirely on microtasks, so it never yields: the tab stops responding, a core
  runs flat out, and nothing short of closing it helps. It takes only an answer without the
  expected field — a command not yet registered while the kernel starts, or an older build.

  - **Internal log** — `charger()` left `cache` empty when the reply had no journal. It now
    always leaves a usable shape, and a failed read says so with a **Try again** button
    instead of an endless "Reading the log…".
  - **Settings** — a reply without `config` re-asked for it on every render. One request at
    a time now, and no redraw unless the answer actually brought something.
  - **Security** — `cache = res` assumed the reply carried `findings`; `render()` then
    counted `cache.findings.length` and threw mid-draw, leaving the view half-painted. The
    reply is normalised before use.

- **The batch-rendered lists stopped loading, permanently.** An `IntersectionObserver` only
  fires when the state *changes*. When a batch was not enough to push the sentinel out of
  the visible zone — short items, tall window, a scrolling ancestor guessed wrong — it
  stayed "visible": no change, so no callback, and scrolling did not help either, since
  scrolling did not change its state any more than the batch had.

  Measured on 3 000 items: **300 were posted, and no amount of scrolling produced the other
  2 700.** The module now keeps posting while the sentinel stays in the zone, measuring
  instead of waiting for a callback that will never come. An empty list no longer leaves its
  sentinel behind either.

- **The sidebar had a horizontal scrollbar on every view.** A Firefox sidebar is 350 px
  wide; the header action row is 390 px and carried `flex-shrink: 0`, pushing the page to
  400 px. It may wrap now. And `.cols2` asked for columns of at least 300 px, which
  overflows a 279 px frame by 21 px — `min(300px, 100%)` says the same intention without
  ever exceeding the frame.

### Added

- **`tools/defilement.mjs`** — fills the store with 20 000 requests and scrolls the table a
  thousand steps down and back up in a real browser, checking at each step that the drawn
  rows follow one another with no gap and no repeated index, that they cover the visible
  area, that `scrollHeight` does not move between frames, and that the batch lists keep
  posting. Result: 39 to 52 rows drawn at a time, 0.01 ms per step, nothing to report.

- **`tools/affichage.mjs`** — opens every view at 350, 700, 1100 and 1600 px and looks for a
  page that overflows sideways, an element wider than its frame with nothing able to scroll
  it into view, siblings that overlap when they should stack, and text cut off silently.

- **The rendering suite now counts the commands each view sends the kernel.** A view that
  re-renders itself in a loop asks for hundreds; a healthy one asks for a handful. The
  counter is installed before the first render and cuts the run short past the ceiling —
  **a test that fails is worth infinitely more than a test that hangs**, which is what this
  suite did for nineteen minutes in CI before the cause was found.

- CI jobs carry a `timeout-minutes` now, so a hang fails in ten minutes rather than running
  for six hours.

1180 assertions across six suites.

## [3.10.0] — 2026-09-12

### Fixed

- **The Timeline tab crashed on any request carrying network timings.** A local
  `const t = rec.perf.timings` shadowed the imported translation function of the same
  name, and the next line called `t(label)` — invoking the timings object as if it were a
  function. Every record imported from a HAR file carries that block: DevTools, Charles and
  Fiddler all write it. The tab went blank on opening.

  Six other places shadow `t` without calling it in that scope. The trap was not armed
  there, but it was laid. They are renamed too.

- **A malformed HAR file could leave a half-built row in the table, and blank the tabs that
  read it.** Everything under the `_interceptor` extension was copied straight into the
  record — `rec.ws = extra.websocket`, and so on. A `websocket` worth `{}` was enough for
  `rec.ws.frames.length` to empty the Streams tab three clicks later.

  The import is the only boundary where data written elsewhere enters the store, so it now
  coerces every value to the expected shape, once, rather than defending against it at
  every place that reads it.

  `store.create` registers a row the moment it is created, so an error halfway through left
  a half-filled one in the table — counted as skipped, and plainly visible. It is removed
  now, and a test checks that what the importer reports matches what the store holds.

- **Half the "nothing to show" screens were displayed in French whatever the language.**
  Thirteen views have one; five passed their text through the translator, eight did not —
  and those are the first screens anyone sees after installing: an empty console, in
  French. There is a single builder now, which translates both texts.

- **The tutorial showed English lessons with French buttons and a French checklist.** Its
  twelve lessons have their text in `content-en.js`, but only the title, goal and body were
  transposed — the 43 action buttons and checklist items came from the French file. For the
  one view whose entire job is to welcome someone new, that was the worst place to leave it.

- **Six strings assembled at runtime could never be translated at all** — "12 lignes",
  "1 hote(s)", "0 flux", the chronogram scale, "https (protocole non mesure)". A text built
  by concatenation is never a dictionary key: they are templates now.

### Changed

- **The test harness now imitates a DOM faithfully enough to render the interface.** It
  did not report `nodeType`, so `ui/lib/dom.js` treated every child element as a string and
  built `"[object Object]"` instead. Any test that constructed a view was only ever reading
  its first layer — which is why none of the bugs above had been caught.

  It reports `nodeType` and `firstChild` now, resolves simple selectors inside a node it
  just built, implements `insertBefore`, and returns a plausible geometry instead of zeros
  so the virtual table actually draws rows. `installerPage()` puts the identifiers the real
  HTML pages declare into the document, which is what lets the views render at all.

- **A sixth suite, `rendu.test.mjs`, renders the interface for real.** Sixteen views against
  three captures, the eleven detail tabs, the twenty-two toolbox panels against thirty-two
  hostile inputs, 264 deliberately malformed HAR files — and a fragment-by-fragment
  comparison of both languages, so no French sentence can stay on an English screen.

1167 assertions across six suites.
...
[3.12.0]: https://github.com/isneoz1/interceptor/releases/tag/v3.12.0
[3.11.0]: https://github.com/isneoz1/interceptor/releases/tag/v3.11.0
[3.10.0]: https://github.com/isneoz1/interceptor/releases/tag/v3.10.0
