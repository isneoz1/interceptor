 ...
 * numeriques (&#233; et &#xE9;) n ont pas besoin de table, elles se calculent.
 *
 * HTML5 en definit plus de deux mille, la plupart jamais employees ; celles-ci
 * couvrent ce qui circule vraiment, et le decodeur laisse intact tout nom
 * inconnu plutot que de rendre un caractere invente.
 */
...
const T = {
  ...
  lang: 9001, rang: 9002, image: 8465, real: 8476, alefsym: 8501,
  weierp: 8472, star: 8902, phone: 9742, check: 10003, cross: 10007
};

/** Point de code -> nom d entite, quand il en existe un. */
export const ENTITES_INVERSES = (() => {
  const out = new Map();
  for (const [nom, code] of Object.entries(T)) if (!out.has(code)) out.set(code, nom);
  return out;
})();

/** Nombre de noms reconnus : affiche dans la boite a outils. */
export const NOMBRE_ENTITES = Object.keys(T).length;

/** Point de code d une entite nommee, ou null si le nom est inconnu. */
export function codeDeLEntite(nom) {
  const cle = String(nom || '');
  if (Object.prototype.hasOwnProperty.call(T, cle)) return T[cle];
  // Les noms se comparent aussi sans tenir compte de la casse pour les formes
  // heritees (&AMP;, &Nbsp;), que les navigateurs acceptent encore.
  const bas = cle.toLowerCase();
  for (const [connu, code] of Object.entries(T)) if (connu.toLowerCase() === bas) return code;
  return null;
}
