...
export function renderFoot() {
  ...
  if (!s) { foot.appendChild(el('span', { text: t('Connexion au noyau…') })); return; }

  const add = (label, value, cls) => {
    foot.appendChild(el('span', {}, [t(label) + ' ', el('b', { class: cls || null, text: String(value) })]));
  };
  ...
  foot.appendChild(el('span', { class: 'right' }, [
    s.capturing ? el('b', { class: 'ok', text: t('Capture') }) : el('b', { class: 'ko', text: t('En pause') }),
    '  ·  ' + t('actif depuis') + ' ' + uptime(s.uptimeMs)
  ]));
}
