...
function chart(rows, format, toneOf) {
  ...
  for (const [label, value] of rows) {
    box.appendChild(el('div', { class: 'lab', text: label, title: label }));
...
export function render() {
  ...
  const list = scoped();
  box.appendChild(sec('Synthese du perimetre', list.length + ' requetes observees'));
  ...
  const tile = (value, label, cls) => tiles.appendChild(el('div', { class: 'tile' + (cls ? ' ' + cls : '') }, [
    el('b', { text: String(value) }), el('label', { text: label })
  ]));
