...
verifier('unsafe-inline signale',
  csp.faits.some(f => /unsafe-inline/.test(f)));
verifier('base-uri manquant signale', csp.faits.some(f => /base-uri absent/.test(f)));
verifier('frame-ancestors manquant signale', csp.faits.some(f => /frame-ancestors absent/.test(f)));
...
verifier('politique stricte : aucune permission large',
  cspStricte.faits.some(f => /aucune permission large/.test(f)));

const cspDouble = analyserCsp("script-src 'self'; script-src 'unsafe-eval'");
verifier('directive en double signalee', cspDouble.faits.some(f => /en double/.test(f)));
verifier('directive inconnue signalee',
  analyserCsp('directive-inventee x').faits.some(f => /inconnue/.test(f)));
