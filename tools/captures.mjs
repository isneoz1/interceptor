...
const SITE = 'https://boutique.exemple.fr';
const CDN = 'https://cdn.exemple.fr';
...
requete({ url: SITE + '/', type: 'main_frame', mime: 'text/html', taille: 48213, duree: 214,
  entetesRep: [
    ...
    { name: 'Content-Security-Policy',
      value: "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.exemple.fr; img-src *" },
...
for (const nom of ['banniere.webp', 'produit-01.webp', 'produit-02.webp', 'logo.svg']) {
  requete({ url: CDN + '/img/' + nom, type: 'image', mime: 'image/webp', taille: 24000 + nom.length * 900, duree: 40 });
}

/* --- Appels applicatifs --- */
requete({ methode: 'POST', url: API + '/v2/panier', taille: 312, duree: 168,
  ...
  corpsReq: { size: 58, kind: 'texte', source: 'webRequest', contentType: 'application/json',
    text: '{"article":"REF-8842","quantite":2,"livraison":"express"}' },
  corpsRep: { size: 312, kind: 'texte', source: 'streamFilter', mime: 'application/json',
    text: '{"panier":{"lignes":2,"total":12840,"devise":"EUR"},"promo":null}' } });

requete({ url: API + '/v2/catalogue?page=1&tri=prix', taille: 18422, duree: 92,
  corpsRep: { size: 18422, kind: 'texte', source: 'streamFilter', mime: 'application/json',
    text: '{"articles":[{"ref":"REF-8842","prix":6420},{"ref":"REF-1130","prix":2990}],"total":248}' } });
...
requete({ url: API + '/v2/stock/REF-8842', statut: 404, taille: 74, duree: 41,
  corpsRep: { size: 74, kind: 'texte', source: 'streamFilter', mime: 'application/json',
    text: '{"erreur":"reference inconnue","code":"STOCK_404"}' } });

requete({ methode: 'PUT', url: API + '/v2/profil', statut: 500, taille: 128, duree: 3120,
  entetesReq: [{ name: 'Content-Type', value: 'application/json' }],
  corpsRep: { size: 128, kind: 'texte', source: 'streamFilter', mime: 'application/json',
    text: '{"erreur":"interne","trace":"db timeout"}' } });

/* --- Ce que l analyseur doit voir : chaque cas est demontrable --- */
requete({ url: 'http://vieux.exemple.fr/api/session', scheme: 'http', statut: 200, taille: 96,
...
requete({ url: API + '/v2/public', taille: 220,
  entetesReq: [{ name: 'Origin', value: 'https://tiers-inconnu.fr' }],
  entetesRep: [{ name: 'Access-Control-Allow-Origin', value: 'https://tiers-inconnu.fr' },
    { name: 'Access-Control-Allow-Credentials', value: 'true' }] });

requete({ url: API + '/v2/suivi', taille: 60,
  cookies: { set: [{ name: 'trace', value: 'x9', sameSite: 'None', secure: false, path: '/' }], changed: [] } });

requete({ url: 'https://pixel.pisteur.net/collect?uid=8f21c', type: 'image', mime: 'image/gif',
  taille: 43, ip: '192.0.2.200' });

requete({ url: CDN + '/absent.js', type: 'script', statut: null, mime: '', erreur: 'NS_ERROR_NET_RESET', duree: 5010 });

/* --- Flux temps reel --- */
const flux = requete({ url: 'wss://temps-reel.exemple.fr/socket', type: 'websocket', statut: 101,
  mime: '', taille: 0, duree: 0 });
flux.ws = {
  frames: [
    { dir: 'out', opcode: 1, ts: horloge + 200, size: 38, text: '{"type":"abonnement","canal":"stock"}' },
    { dir: 'in', opcode: 1, ts: horloge + 420, size: 64, text: '{"canal":"stock","ref":"REF-8842","restant":7}' },
    { dir: 'in', opcode: 1, ts: horloge + 1900, size: 63, text: '{"canal":"stock","ref":"REF-1130","restant":0}' },
    { dir: 'out', opcode: 9, ts: horloge + 3000, size: 0, text: '' }
  ]
};
store.touch(flux.id);

const sse = requete({ url: API + '/v2/evenements', type: 'other', mime: 'text/event-stream',
  taille: 512, duree: 0 });
sse.sse = {
  messages: [
    { event: 'commande', data: '{"id":"CMD-77","etat":"payee"}', ts: horloge + 800 },
    { event: 'commande', data: '{"id":"CMD-78","etat":"preparation"}', ts: horloge + 4200 }
  ]
};
store.touch(sse.id);
...
await config.set({
  rulesEnabled: false,
  rules: [
    { id: 'r-pisteurs', enabled: true, name: 'Bloquer les pisteurs connus',
      match: { urlRegex: 'pisteur\\.net|doubleclick\\.net|google-analytics\\.com' }, action: 'block' },
    { id: 'r-lent', enabled: true, name: 'Simuler un reseau lent sur l API',
      match: { host: 'api.exemple.fr' }, action: 'delay', delayMs: 750 },
    { id: 'r-stock', enabled: false, name: 'Repondre 200 sur le stock',
      match: { urlRegex: '/v2/stock/' }, action: 'mock',
      mock: { body: '{"restant":42}', contentType: 'application/json' } }
  ]
});
...
(() => {
  ...
  const api = {
    ...
    tabs: { query: async () => [{ id: 3, title: 'boutique.exemple.fr', url: 'https://boutique.exemple.fr/' }],
...
if (page1) {
  ...
  await page('Runtime.evaluate', {
    expression: '(() => { const b = [...document.querySelectorAll("#dtabs .dtab")]'
      + '.find(x => /En-tetes/.test(x.textContent)); if (b) b.click(); })()'
  });
  await patienter(500);
